		...
		<?php if ( have_comments() ) : ?>
			...
			<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
				...
				paginate_comments_links( apply_filters( 'woocommerce_comment_pagination_args', array(
					'prev_text' => esc_html(__('&larr;', 'artday')),
					'next_text' => esc_html(__('&rarr;', 'artday')),
					'type'      => 'list',
				) ) );
				echo '</nav>';
			endif; ?>
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					$comment_form = array(
						'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'woocommerce' ). '<div class="ws-separator"></div>' : esc_html__( 'Be the first to review', 'woocommerce' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
					...
					if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
						$comment_form['must_log_in'] = '<p class="must-log-in">' .  sprintf( wp_kses_post(__( 'You must be <a href="%s">logged in</a> to post a review.', 'woocommerce' )), esc_url( $account_page_url ) ) . '</p>';
					}
