...
class ulp_klaviyo_class {
	...
	function __construct() {
		$this->default_popup_options['klaviyo_fields'] = serialize($this->fields);
		if (is_admin()) {
	...
	function popup_options_show($_popup_options) {
		...
		if ($ulp->ext_options['enable_customfields'] == 'on') {
			$fields = unserialize($popup_options['klaviyo_fields']);
		...
		if ($ulp->ext_options['enable_customfields'] == 'on') {
			...
			foreach($this->fields as $key => $value) {
				if (isset($ulp->postdata['ulp_klaviyo_field_'.$key])) {
					$fields[$key] = stripslashes(trim($ulp->postdata['ulp_klaviyo_field_'.$key]));
				}
			}
			$popup_options['klaviyo_fields'] = serialize($fields);
		}
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['klaviyo_enable'] == 'on') {
			...
			if ($ulp->ext_options['enable_customfields'] == 'on') {
				$fields = unserialize($_popup_options['klaviyo_fields']);
				if (is_array($fields)) $fields = array_merge($this->fields, $fields);
				else $fields = $this->fields;
				...
				if (!empty($_subscriber['{subscription-phone}'])) $properties['$'.'phone_number'] = $_subscriber['{subscription-phone}'];
			}
