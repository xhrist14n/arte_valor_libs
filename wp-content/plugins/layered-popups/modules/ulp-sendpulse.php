...
class ulp_sendpulse_class {
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['sendpulse_enable'] == 'on') {
			...
			try {
				$curl = curl_init('http://api.sendpulse.com/oauth/access_token');
				...
				if ($json && is_array($json) && array_key_exists('access_token', $json)){
					...
					$curl = curl_init('http://api.sendpulse.com/addressbooks/'.$popup_options['sendpulse_list_id'].'/emails');
					curl_setopt($curl, CURLOPT_POST, 1);
					curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($data));
		...
		if (current_user_can('manage_options')) {
			...
			try {
				...
				if ($json && is_array($json) && array_key_exists('access_token', $json)){
					...
					$curl = curl_init('http://api.sendpulse.com/addressbooks');
					curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
					curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
					$response = curl_exec($curl);
					curl_close($curl);
