...
<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
	...
	function front_popup_content($_content, $_popup_options) {
		global $ulp;
		$popup_options = array_merge($this->default_popup_options, $_popup_options);
		$facebook_button = '<a class="ulp-submit-button ulp-submit-facebook'.($popup_options['button_inherit_size'] == 'on' ? ' ulp-inherited' : '').'" onclick="return ulp_subscribe_facebook(this);"'.($ulp->options['fa_enable'] == 'on' ? ' data-fa="fa-facebook"' : '').' data-label="'.esc_html($popup_options['social2_facebook_label']).'" data-loading="'.esc_html($popup_options['button_label_loading']).'">'.($ulp->options['fa_enable'] == 'on' ? '<i class="fa fa-facebook"></i>&nbsp; ' : '').esc_html($popup_options['social2_facebook_label']).'</a>';
		$google_button = '<a class="ulp-submit-button ulp-submit-google'.($popup_options['button_inherit_size'] == 'on' ? ' ulp-inherited' : '').'" onclick="return ulp_subscribe_google(this);"'.($ulp->options['fa_enable'] == 'on' ? ' data-fa="fa-google"' : '').' data-label="'.esc_html($popup_options['social2_google_label']).'" data-loading="'.esc_html($popup_options['button_label_loading']).'">'.($ulp->options['fa_enable'] == 'on' ? '<i class="fa fa-google"></i>&nbsp; ' : '').esc_html($popup_options['social2_google_label']).'</a>';
