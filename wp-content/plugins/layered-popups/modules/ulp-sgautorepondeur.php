...
class ulp_sgautorepondeur_class {
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['sgautorepondeur_enable'] == 'on') {
			...
			$result = $this->connect('inscription', $data);
		}
	}
	function show_lists() {
		global $wpdb;
		if (current_user_can('manage_options')) {
			...
			$result = $this->connect('listes', array('membreid' => $member_id, 'codeactivation' => $code));

			if (is_array($result)) {
				if (array_key_exists('valid', $result) && $result['valid']) {
					foreach ($result['data'] as $list) {
						if (is_array($list)) {
							if (array_key_exists('id', $list) && array_key_exists('nom', $list)) {
	...
	function connect($_path, $_data) {
		try {
			$url = 'http://sg-autorepondeur.com/api/'.ltrim($_path, '/');
			$curl = curl_init($url);
