...
class ulp_welcomemail_class {
	...
	function popup_options_check($_errors) {
		...
		if ($popup_options['welcomemail_enable'] == 'on') {
			...
			else if (strlen($popup_options['welcomemail_subject']) > 64) $errors[] = __('Welcome E-mail subject must contain maximum 64 characters', 'ulp');
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		if ($popup_options['welcomemail_enable'] == 'on') {
			...
			wp_mail($_subscriber['{subscription-email}'], $popup_options['welcomemail_subject'], $body, $mail_headers);
		}
	}
}
