		...
		if ($popup_options['mail_enable'] == 'on') {
			...
			if (!empty($_subscriber['{subscription-email}'])) {
				...
				if ($popup_options['mail_from'] == 'on') {
					$from_set = true;
					$mail_headers .= "From: ".(empty($_subscriber['{subscription-name}']) ? esc_html($_subscriber['{subscription-email}']) : esc_html($_subscriber['{subscription-name}']))." <".esc_html($_subscriber['{subscription-email}']).">\r\n";
				}
			}
