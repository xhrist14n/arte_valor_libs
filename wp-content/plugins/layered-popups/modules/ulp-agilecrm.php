...
class ulp_agilecrm_class {
	...
	function __construct() {
		$this->default_popup_options['agilecrm_fields'] = serialize($this->fields);
	...
	function popup_options_show($_popup_options) {
				...
				<table class="ulp_useroptions">
					...
					<tr>
						...
						<td>
							...
							<br /><em>'.__('Enter your AgileCRM REST API Key. Go to your AgileCRM account, click "Admin Settings" and "API & Analytics".', 'ulp').'</em>
						</td>
					</tr>
					<tr>
		...
		$fields = unserialize($popup_options['agilecrm_fields']);
		...
		foreach ($this->fields as $key => $value) {
			echo '
									<tr>
										...
										<td>
											<input type="text" id="ulp_agilecrm_field_'.esc_html($key).'" name="ulp_agilecrm_field_'.esc_html($key).'" value="'.esc_html($fields[$key]).'" class="widefat"'.($key == 'email' ? ' readonly="readonly"' : '').' />
		...
		if ($ulp->ext_options['enable_customfields'] == 'on') {
			...
			foreach($this->fields as $key => $value) {
				if (isset($ulp->postdata['ulp_agilecrm_field_'.$key])) {
					$fields[$key] = stripslashes(trim($ulp->postdata['ulp_agilecrm_field_'.$key]));
				}
			}
			$popup_options['agilecrm_fields'] = serialize($fields);
		}
	...
	function subscribe($_popup_options, $_subscriber) {
		...
		$popup_options = array_merge($this->default_popup_options, $_popup_options);
		if ($popup_options['agilecrm_enable'] == 'on') {
			$fields = unserialize($_popup_options['agilecrm_fields']);
			if (is_array($fields)) $fields = array_merge($this->fields, $fields);
			else $fields = $this->fields;
			$data = array(
