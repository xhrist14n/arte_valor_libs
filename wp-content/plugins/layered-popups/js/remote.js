...
function ulp_add_event(event, data) {
	if (jQuery.isPlainObject(data)) {
		if (!data.popup && !data.popup_mobile) return;
		var popup;
		if (ulp_mobile && data.popup_mobile) popup = data.popup_mobile;
		else popup = data.popup;
		if (jQuery(popup).is(":empty")) return;
		switch (event) {
			case "onload":
			...
			case "onscroll":
				...
				if (typeof data.offset !== 'undefined') ulp_onscroll_offset = data.offset;
				break;
			case "onidle":
